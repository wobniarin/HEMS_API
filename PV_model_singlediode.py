"""
Calculating a module's IV curves
================================

Examples of modeling IV curves using a single-diode circuit equivalent model.
"""

# 
# Calculating a module IV curve for certain operating conditions is a two-step
# process.  Multiple methods exist for both parts of the process.  Here we use
# the De Soto model [1]_ to calculate the electrical parameters for an IV
# curve at a certain irradiance and temperature using the module's
# base characteristics at reference conditions.  Those parameters are then used
# to calculate the module's IV curve by solving the single-diode equation using
# the Lambert W method.
#
# The single-diode equation is a circuit-equivalent model of a PV
# cell and has five electrical parameters that depend on the operating
# conditions.  For more details on the single-diode equation and the five
# parameters, see the `PVPMC single diode page
# <https://pvpmc.sandia.gov/modeling-steps/2-dc-module-iv/diode-equivalent-circuit-models/>`_.
#
# References
# ----------
#  .. [1] W. De Soto et al., "Improvement and validation of a model for
#     photovoltaic array performance", Solar Energy, vol 80, pp. 78-88, 2006.
#
# Calculating IV Curves
# -----------------------
# This example uses :py:meth:`pvlib.pvsystem.calcparams_desoto` to calculate
# the 5 electrical parameters needed to solve the single-diode equation.
# :py:meth:`pvlib.pvsystem.singlediode` is then used to generate the IV curves.

from pvlib import pvsystem, temperature
import pandas as pd
import matplotlib.pyplot as plt
from irradiance import irradiation
from BBDD import BBDD

class PV:
    def __init__(self,num_panels,corrected: int=0, module_name: str='LONGi_Green_Energy_Technology_Co___Ltd__LR6_72PE_360M'):
        self.corrected=corrected
        
        if corrected==0:
            self.name_BBDD='forecast_weather_BBDD'
            self.back_days=0
            self.ndays=5
        else:
            self.name_BBDD='current_weather_BBDD'
            self.back_days=2
            self.ndays=2
            
        #print(1)
        # Example module parameters for the Canadian Solar CS5P-220M:
        moddb = pvsystem.retrieve_sam(name='CECMod')
        #print(2)
        #module_name='JA_Solar_JAM72S01_335_PR'
        self.parameters = dict(moddb[module_name])
        #print(3)
        self.parameters.update({'Name':module_name})
        #print(4)
        self.num_panels=num_panels
        #print(5)

    def ambient_inputs(self):
        irr=irradiation()
        

        ambient_inp=irr.get_params_PV_forecast_prod(self.name_BBDD,self.timestamp,-self.back_days,ndays=self.ndays)
        
    
        return ambient_inp
        

    def conditions(self):
        
        df=self.ambient_inputs()
        
        temp_cell=temperature.sapm_cell(df['eff_irrad'], df['main temp']-273.15, df['wind speed'], a=-3.56, b=-0.075, deltaT=0, irrad_ref=1000)
        temp_cell=temp_cell.rename('Tcell')
        conditions=df.join(temp_cell, lsuffix='_caller', rsuffix='_other')

        self.conditions_df=conditions


    def desoto_calculation(self):
        
        self.conditions()
        # adjust the reference parameters according to the operating
        # conditions using the De Soto model:
        IL, I0, Rs, Rsh, nNsVth = pvsystem.calcparams_desoto(
            self.conditions_df['eff_irrad'],
            self.conditions_df['Tcell'],
            alpha_sc=self.parameters['alpha_sc'],
            a_ref=self.parameters['a_ref'],
            I_L_ref=self.parameters['I_L_ref'],
            I_o_ref=self.parameters['I_o_ref'],
            R_sh_ref=self.parameters['R_sh_ref'],
            R_s=self.parameters['R_s'],
            EgRef=1.121,
            dEgdT=-0.0002677
        )
        
        return IL, I0, Rs, Rsh, nNsVth

    def PV_curves_info(self, ivcurve_pnts):
        IL, I0, Rs, Rsh, nNsVth=self.desoto_calculation()
        # plug the parameters into the SDE and solve for IV curves:
        curve_info = pvsystem.singlediode(
            photocurrent=IL,
            saturation_current=I0,
            resistance_series=Rs,
            resistance_shunt=Rsh,
            nNsVth=nNsVth,
            ivcurve_pnts=ivcurve_pnts,
            method='lambertw'
        )
        return curve_info
        
    def sim_PV_power(self,timestamp: int=15):
        
        #Primer posem el if que ens diferenciarà els noms de la BBDD a la que guardarem les dades segons sí és la corregida o la previsió i també el nom de la columna
        #és important lo del nom de la columna ja que sinó el plotter no funciona correctament si tenim dues columnes amb el mateix nom
        
        if self.corrected==0:
            new_BBDD='PV_forecast_BBDD'
            power_column='p_mp'
        else:
            new_BBDD='PV_corr_forecast_BBDD'
            power_column='corr_p_mp'
        
        self.timestamp=timestamp
        curve_info=self.PV_curves_info(40)
        #del curve_info['i']
        #del curve_info['v']
        #data=pd.DataFrame.from_dict(curve_info)
        curve_info['v_oc']=curve_info['v_oc']*self.num_panels
        curve_info['v_mp']=curve_info['v_mp']*self.num_panels
        curve_info['p_mp']=curve_info['p_mp']*self.num_panels/1000
        
        if power_column!='p_mp':  
            curve_info[power_column] = curve_info['p_mp']
            del curve_info['p_mp']
        
        curve_info['dt']=self.conditions_df.index
        
        p_mp={'dt':curve_info['dt'],power_column:curve_info[power_column]}
        
        data=[{k:curve_info[k][i] for k in curve_info}for i in range(len(curve_info['v_oc']))]


        self.store_data(data,new_BBDD)

        
        return p_mp
    
    
    
    def store_data(self,data,name):
        this_BBDD=BBDD(name)
        this_BBDD.store_data(data)
        
        
    def plot_curves(self):
        conditions_df_red=self.conditions_df.head(5)
        curve_info=self.PV_curves_info(40)
        # plot the calculated curves:
        plt.figure()
        for i, case in conditions_df_red.iterrows():
            label = (
                'a'
                'b'
            )
            plt.plot(curve_info['v'][i], curve_info['i'][i])
            v_mp = curve_info['v_mp'][i]
            i_mp = curve_info['i_mp'][i]
            # mark the MPP
            plt.plot([v_mp], [i_mp], ls='', marker='o', c='k')
        
        plt.legend(loc=(1.0, 0))
        plt.xlabel('Module voltage [V]')
        plt.ylabel('Module current [A]')
        plt.title(self.parameters['Name'])
        plt.show()
        plt.gcf().set_tight_layout(True)
        
        
        # draw trend arrows
        def draw_arrow(ax, label, x0, y0, rotation, size, direction):
            style = direction + 'arrow'
            bbox_props = dict(boxstyle=style, fc=(0.8, 0.9, 0.9), ec="b", lw=1)
            t = ax.text(x0, y0, label, ha="left", va="bottom", rotation=rotation,
                        size=size, bbox=bbox_props, zorder=-1)
        
            bb = t.get_bbox_patch()
            bb.set_boxstyle(style, pad=0.6)
        
        
        ax = plt.gca()
        draw_arrow(ax, 'Irradiance', 20, 2.5, 90, 15, 'r')
        draw_arrow(ax, 'Temperature', 35, 1, 0, 15, 'l')
        
        print(pd.DataFrame({
            'i_sc': curve_info['i_sc'],
            'v_oc': curve_info['v_oc'],
            'i_mp': curve_info['i_mp'],
            'v_mp': curve_info['v_mp'],
            'p_mp': curve_info['p_mp'],
        }))


if __name__=='__main__':
    
    puig_pv=PV(num_panels=12, corrected=1)

    DC_output_power=puig_pv.sim_PV_power(timestamp=15)
    conditions=puig_pv.conditions()
    #plt.plot(DC_output_power['p_mp'])
    #puig_pv.plot_curves()
    #from analytics import scoring
    #sc=scoring()
    #pv_corr_scores=sc.some_scores(BBDDs=['grid_BBDD','PV_corr_forecast_BBDD'],Columns=['P_PV [kW]','corr_p_mp'],frequencies=5, signs=[1,1],days_from_today=-8,ndays=1)
    
    
    
    
    