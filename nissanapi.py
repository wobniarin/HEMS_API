...
class KamereonSession:
    ...
    def login(self, username, password):
        ...
        self._oauth = OAuth2Session(
            ...
            redirect_uri=self.settings['redirect_uri'],
            scope=self.settings['scope'])
        #print('aqui faig prints')
        #print(self.settings['client_id'])
        #print(self.settings['redirect_uri'])
        #print(self.settings['scope'])
...
class Vehicle:
    ...
    def fetch_lock_status(self):
        ...
        self.lock_status_last_updated = datetime.datetime.fromisoformat(self.location_data['lastUpdateTime'].replace('Z','+00:00'))
        return 'aqui sí hi és'
    ...
    def fetch_charging_consumption_history(self):
        resp = self.session.oauth.get(
            '{}v1/cars/{}/consumption-history'.format(self.session.settings['car_adapter_base_url'], self.vin),
            headers={'Content-Type': 'application/vnd.api+json'}
        )
...
class car_data_recollect():
    def __init__(self):
        ...
        self.user_id = nci.get_user_id()
        self.vehicles = nci.fetch_vehicles()
    ...
    def data(self):
        ...
        vehicle.refresh_battery_status()
        vehicle.fetch_battery_status()
        vehicle.refresh_location()
        vehicle.fetch_location()
        
        if 'PluggedStatus.NOT_PLUGGED'==str(vehicle.plugged_in):
            plug_status=0
        else:
            plug_status=1
        
        if 'ChargingStatus.NOT_CHARGING'==str(vehicle.charging):
            charging_status=0
        else:
            charging_status=1        

        
        row=[dt.datetime.now(),vehicle.battery_status_last_updated + dt.timedelta(hours=2),vehicle.battery_level,charging_status,plug_status,vehicle.location_last_updated + dt.timedelta(hours=2),vehicle.total_mileage,vehicle.charge_time_required_to_full,str(vehicle.charging_speed),vehicle.instantaneous_power,vehicle.battery_temperature,vehicle.battery_capacity]
        names=['dt','dt battery update','battery level','charging status','plug status','dt location update','mileage','time to full','charging speed','instant power','battery temperature','battery capacity']
        
        data=[(dict(zip(names,row)))]
        self.store_data(data,'car_BBDD')
...
if __name__ == '__main__':
    ...
    nissan=car_data_recollect()
    print(nissan.vehicles)
    a=nissan.data()
    ...
    while True:
        ...
        vehicle.fetch_location()
        if a==1:
            vehicle.control_charging('start')
            print("ara li demano que carregui")
            #a=0
        elif a==0:
            vehicle.control_charging('stop')
            print("ara li demano que pari de carregar")
            #a=1
        #vehicle.control_horn_lights('start','lights')
        print(vehicle.battery_status_last_updated)
        print(vehicle.battery_level)
        print(vehicle.location_last_updated)
        print(vehicle.location)       
        print(vehicle.charging)
        print(vehicle.plugged_in)
        print(vehicle.battery_temperature)
        names=['dt','dt battery update','battery level',]
        row=[dt.datetime.now(),vehicle.battery_status_last_updated,vehicle.battery_level,vehicle.charging,vehicle.plugged_in,vehicle.location_last_updated,vehicle.location]
        #print(row)
